	...
	case ar_storage:read_term(data_sync_state) of
		{ok, #{ block_index := RecentBI, disk_pool_data_roots := DiskPoolDataRoots } = M} ->
			maps:merge(M, #{
				...
				disk_pool_size => calculate_disk_pool_size(DiskPoolDataRoots),
	...
	case is_disk_pool_chunk(AbsoluteOffset, ChunkDataKey, State) of
		false ->
			...
			?LOG_DEBUG([{event, chunk_already_moved_from_disk_pool}, {offset, AbsoluteOffset},
					{chunk_data_key, ar_util:encode(element(5, Args))},
					{data_root, ar_util:encode(DataRoot)}]),
			gen_server:cast(self(), {process_disk_pool_chunk_offsets, Iterator, MayConclude,
					Args}),
			{noreply, State2};
		{error, Reason} ->
		...
		true ->
			case maps:is_key(AbsoluteOffset, PackingMap) of
				...
				false ->
					case ar_packing_server:is_buffer_full() of
						true ->
							gen_server:cast(self(), {process_disk_pool_chunk_offsets,
									Iterator, false, Args}),
							{noreply, State};
						false ->
							case read_chunk(AbsoluteOffset, ChunkDataDB,
									ChunkDataKey, StoreID) of
								not_found ->
								...
								{ok, {Chunk, DataPath}} ->
									...
									PackingArgs = {pack_disk_pool_chunk, ChunkDataKey,
											{RequiredPacking, DataPath, Offset, DataRoot,
											TXPath}},
									gen_server:cast(self(), {process_disk_pool_chunk_offsets,
											Iterator, false, Args}),
									{noreply, State#sync_data_state{
